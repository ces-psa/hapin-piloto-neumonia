...
pneumo_classified_old <- pneumo_visits %>%
  ...
  mutate(
    ...
    tachypnea = case_when(
      ...
      age_group == "12 - <60 months"  & rr >= 40 ~ TRUE,
      is.na(rr) ~ NA,
      TRUE ~ FALSE
    ),
...
pneumo_classified %>%
  ...
  gather(key = pneumo, value = value, ari, who_pneumonia, who_pneumonia_severe, hapin_pneumonia) %>%
  filter(value) %>%
...
pneumo_classified %>%
  ...
  gather(
    ...
    who_pneumonia, severe_who_pneumonia, hapin_pneumonia,
    factor_key = TRUE
  ) %>%
  filter(value) %>%
  group_by(case_group) %>%
  mutate(
    n = n(),
    case_label = recode_factor(case_group, !!!case_names_sp)
  ) %>%
  ungroup() %>%
  select(record_id, case_group, everything(), -value) %>%
  gather(
    key = symptom, value = present,
    -record_id, -case_group, -case_label, -n,
    factor_key = TRUE
  ) %>%
  mutate(
    symptom = recode_factor(
      symptom,
      tachypnea = "Taquipnea",
      chest_indrawing = "Tiraje",
      danger_signs = "Signos de peligro",
      hypoxemia = "Hipoxemia",
      lus_pneumonia = "Neumonía por ultrasonido"
    )
  ) %>%
  group_by(case_label, n, symptom) %>%
  summarize(
...
pneumo_classified %>%
  ...
  gather(group, value, factor_key = TRUE) %>%
  count(group, value) %>%
  ...
  mutate(
    ...
    group = fct_relabel(
      ...
      case_type2 = "WHO pneumonia",
      case_type3 = "WHO severe pneumonia",
      case_type4 = "HAPIN pneumonia",
      definition1 = "a) Cough or b) difficulty breathing",
      definition2 = "a) Chest indrawing or b) tachypnea",
      definition3 = "a) Danger signs",
      definition4 = "a) Hypoxemia or b) consolidation"
    ),
    value = factor(
      value,
      levels = c(c(
...
pneumo_weeks <- pneumo_classified %>%
  ...
  count(service_type, case_group, week) %>%
...
surveillance_weeks %>%
  ...
  mutate(
    Surveillance = paste0(person_hours, " (", days, ")")
  ) %>%
  select(
    "Service type" = service_type,
    "Surveillance effort" = Surveillance,
...
weekly_cases <- surveillance_weeks %>%
  full_join(pneumo_weeks) %>%
  gather(case_group, value, who_pneumonia, severe_who_pneumonia, hapin_pneumonia) %>%
  mutate(
...
pneumo_classified %>%
  filter(case_group %in% c("hapin_pneumonia", "severe_who_pneumonia")) %>%
  select(
